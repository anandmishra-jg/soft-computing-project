...
import copy
import sys
sys.path.append("/content/drive/Shareddrives/SC project/EvolutionaryComputing-main/data")
...
def get_fitness(total_pop, train_ind, train_img, train_lab, val_ind, epochs, lr, num_classes, weights):
    ...
    for i in range(len(total_pop)):
      ...
      device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
      acc = train(net.to(device), train_ind, train_img, train_lab, val_ind, optimizer, epochs, weights)
      fitness[i] = acc
      print(f'fitness {i} : {acc}')
    return fitness
...
def tournament(total_pop, fitness, tournament_size, matpool_size):
    ...
    while len(selected_to_mate) < matpool_size:
      ...
      ts_ind = np.random.choice(hash_ind, tournament_size, replace = True) # randomly select (tournament_size) parents, return parent index
      #print(ts_ind)
...
def run():
    ...
    # np.random.seed(41)
    # random.seed(41)
    file = "/content/drive/Shareddrives/SC project/EvolutionaryComputing-main/data/y_train_covid.csv"
    ...
    population = population.tolist()[:10]
